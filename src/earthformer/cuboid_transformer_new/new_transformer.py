...
class NewTransformerModel(nn.Module):
    ...
    def __init__(self,
        ...
        llama_dim = 4096
        # # llama4vision
        llama_default_config = {"dim": llama_dim, "multiple_of": 256,
                                "n_heads": 32, "n_layers": 16, "norm_eps": 1.0e-6,
                                "vocab_size": -1, "first_layer": 14}
        self.llama = LLaMATransformer(llama_default_config)
        self.llama_dim_mapper1 = nn.Linear(mem_shapes[-1][-1], llama_dim, bias=False)
        self.llama_dim_mapper2 = nn.Linear(llama_dim, mem_shapes[-1][-1], bias=False)
        import time
        ckpt_path = "/root/autodl-tmp/models--nyanko7--LLaMA-7B/snapshots/3b94aad3d8e39981b81e14239446d935b6dbf2bd/consolidated.00.pth"
        ...
        self.FPG_module = FPG(img_size=H_out,
                              patch_size=16,
                              in_channels=1,
                              out_channels=1,
                              embed_dim=128,
    ...
    def forward(self, x, t=None, y=None, verbose=False):
        """
        # 这是一个前向传播函数，用于计算输出张量
        Parameters
        ----------
        x
            # 输入张量，形状为 (B, T, H, W, C)，其中B是批次大小，T是时间步长，H是高度，W是宽度，C是通道数
        t
            # 目标张量，形状为 (B, T, H, W, C)，可选参数，如果没有提供，则默认为None
        y
            # 预测张量，形状为 (B, T, H, W, C)，可选参数，如果没有提供，则默认为None
        verbose
            # 布尔值，表示是否打印中间张量的形状，可选参数，如果没有提供，则默认为False
        Returns
        -------
        out
            # 输出张量，形状为 (B, T_out, H, W, C_out)，其中T_out是目标时间步长，C_out是输出通道数
        """
        B, T, _, _, _ = x.shape  # 从输入张量中获取批次大小和时间步长

        x_fpg = x.permute(0, 1, 4, 3, 2) # (B, T, C, W, H)
        pde_features = self.FPG_module(x_fpg)
        T_out = self.target_shape[0]  # 从模型属性中获取目标时间步长
        x = self.initial_encoder(x)  # 对输入张量进行初始编码，将其形状从 (B, T, 384, 384, 1) 变为 (B, T, 32, 32, 128)
        x = self.enc_pos_embed(x)  # 对编码后的输入张量添加位置嵌入


        t = t.float()  # 将目标张量转换为浮点类型
        t = self.initial_encoder(t)  # 对目标张量进行初始编码，将其形状从 (B, T, 384, 384, 1) 变为 (B, T, 32, 32, 128)
        t = self.enc_pos_embed(t)  # 对编码后的目标张量添加位置嵌入

        y = y.float()  # 将预测张量转换为浮点类型
        if T_out < T :  # 如果目标时间步长大于输入时间步长，并且预测张量不为空
            pde_features = pde_features[:, :-1, :, :, :]  # 则去掉预测张量的最后一个时间步
        # y = self.initial_encoder(y)  # 对预测张量进行初始编码，将其形状从 (B, T, 384, 384, 1) 变为 (B, T, 32, 32, 128)
        # y = self.enc_pos_embed(y)  # 对编码后的预测张量添加位置嵌入
        # x = x + t + y
        # x = x + y  # 将编码后的输入、目标和预测张量相加，得到一个形状为 (B, T, 32, 32, 128) 的张量
        # x = t + y
        # x = x   # 将编码后的输入、目标和预测张量相加，得到一个形状为 (B, T, 32, 32, 128) 的张量
        if self.num_global_vectors > 0:  # 如果模型属性中指定了全局向量的数量
            init_global_vectors = self.init_global_vectors \
                .expand(B, self.num_global_vectors, self.global_dim_ratio * self.base_units)  # 则根据批次大小和全局维度比例扩展初始的全局向量
            mem_l, mem_global_vector_l = self.encoder(x, init_global_vectors)  # 并将其作为参数传入编码器，得到编码器的输出和全局向量的输出
        else:  # 否则
            mem_l = self.encoder(x)  # 只传入编码器的输入，得到编码器的输出
        if verbose:  # 如果指定了打印中间张量的形状
            for i, mem in enumerate(mem_l):  # 则遍历编码器的输出列表
                print(f"mem[{i}].shape = {mem.shape}")  # 并打印每个张量的形状

        # mem_l_y = self.encoder(y)
        # mem_l = [mem + mem_y for mem, mem_y in zip(mem_l, mem_l_y)]
        initial_z = self.get_initial_z(final_mem=mem_l[-1],
                                       T_out=T_out)  # 从编码器的最后一个输出和目标时间步长中获取初始的隐变量
        # # Llama model
        B_m, T_m, W_m, H_m, C_m = mem_l[-1].shape  # 从编码器的最后一个输出中获取批次大小、时间步长、宽度、高度和通道数
        mel_tmp = mem_l[-1].contiguous().view(B, -1, C_m)  # 将编码器的最后一个输出转换为连续的张量，并将其形状从 (B, T, W, H, C) 变为 (B, T*W*H, C)
        mel_tmp = self.llama_dim_mapper1(mel_tmp)  # 对转换后的张量进行维度映射，将其通道数从 C 变为 C/2
        llama_output = self.llama(mel_tmp)  # 将维度映射后的张量传入LLAMA模块，得到LLAMA的输出
        llama_output = self.llama_dim_mapper2(llama_output)  # 对LLAMA的输出进行维度映射，将其通道数从 C/2 变为 C
        mem_l[-1] = llama_output.view(B_m, T_m, W_m, H_m, C_m)  # 将LLAMA的输出重新变形为 (B, T, W, H, C)，并替换编码器的最后一个输出


        if self.num_global_vectors > 0:  # 如果模型属性中指定了全局向量的数量
            dec_out = self.decoder(initial_z, mem_l, mem_global_vector_l)  # 则将初始的隐变量、编码器的输出和全局向量的输出作为参数传入解码器，得到解码器的输出
        else:  # 否则
            dec_out = self.decoder(initial_z, mem_l)  # 只传入初始的隐变量和编码器的输出，得到解码器的输出
        dec_out = self.final_decoder(dec_out)  # 对解码器的输出进行最终的解码，得到一个形状为 (B, T_out, 32, 32, 128) 的张量
        out = self.dec_final_proj(dec_out)  # 对最终解码后的张量进行投影，将其形状从 (B, T_out, 32, 32, 128) 变为 (B, T_out, 384, 384, 1) B T H W C
        out = out + pde_features.permute(0, 1, 4, 3, 2) + y
        return out  # 返回输出张量
