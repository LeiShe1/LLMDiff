...
class NewTransformerModel(nn.Module):
    ...
    def forward(self, x, t=None, y=None, verbose=False):
        ...
        # y_out = y
        if T_out < T and y is not None:  # 如果目标时间步长大于输入时间步长，并且预测张量不为空
            y_out = y[:, :-1, :, :, :]  # 则去掉预测张量的最后一个时间步
        else:
            y_out = y
        y = self.initial_encoder(y)  # 对预测张量进行初始编码，将其形状从 (B, T, 384, 384, 1) 变为 (B, T, 32, 32, 128)
        y = self.enc_pos_embed(y)  # 对编码后的预测张量添加位置嵌入

        # x = x + t + y  # 将编码后的输入、目标和预测张量相加，得到一个形状为 (B, T, 32, 32, 128) 的张量
        x = x + y
        # x = t + y
        ...
        out = self.dec_final_proj(dec_out)  # 对最终解码后的张量进行投影，将其形状从 (B, T_out, 32, 32, 128) 变为 (B, T_out, 384, 384, 1)
        out = 0.1*out + y_out
        return out  # 返回输出张量
