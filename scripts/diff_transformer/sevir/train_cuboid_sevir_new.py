...
from earthformer.cuboid_transformer.new_transformer import NewTransformerModel
...
from earthformer.diffusion import create_diffusion
...
class CuboidSEVIRPLModule(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.torch_nn_module = NewTransformerModel(
        ...
        earthformer_model_cfg = OmegaConf.to_object(oc.model.earthformer)
        self.first_stage_model = CuboidTransformerModel(
            input_shape=earthformer_model_cfg["input_shape"],
            target_shape=earthformer_model_cfg["target_shape"],
            base_units=earthformer_model_cfg["base_units"],
            scale_alpha=earthformer_model_cfg["scale_alpha"],
            enc_depth=earthformer_model_cfg["enc_depth"],
            dec_depth=earthformer_model_cfg["dec_depth"],
            enc_use_inter_ffn=earthformer_model_cfg["enc_use_inter_ffn"],
            dec_use_inter_ffn=earthformer_model_cfg["dec_use_inter_ffn"],
            dec_hierarchical_pos_embed=earthformer_model_cfg["dec_hierarchical_pos_embed"],
            downsample=earthformer_model_cfg["downsample"],
            downsample_type=earthformer_model_cfg["downsample_type"],
            enc_attn_patterns=enc_attn_patterns,
            dec_self_attn_patterns=dec_self_attn_patterns,
            dec_cross_attn_patterns=dec_cross_attn_patterns,
            dec_cross_last_n_frames=earthformer_model_cfg["dec_cross_last_n_frames"],
            dec_use_first_self_attn=earthformer_model_cfg["dec_use_first_self_attn"],
            num_heads=earthformer_model_cfg["num_heads"],
            attn_drop=earthformer_model_cfg["attn_drop"],
            proj_drop=earthformer_model_cfg["proj_drop"],
            ffn_drop=earthformer_model_cfg["ffn_drop"],
            upsample_type=earthformer_model_cfg["upsample_type"],
            ffn_activation=earthformer_model_cfg["ffn_activation"],
            gated_ffn=earthformer_model_cfg["gated_ffn"],
            norm_layer=earthformer_model_cfg["norm_layer"],
            # global vectors
            num_global_vectors=earthformer_model_cfg["num_global_vectors"],
            use_dec_self_global=earthformer_model_cfg["use_dec_self_global"],
            dec_self_update_global=earthformer_model_cfg["dec_self_update_global"],
            use_dec_cross_global=earthformer_model_cfg["use_dec_cross_global"],
            use_global_vector_ffn=earthformer_model_cfg["use_global_vector_ffn"],
            use_global_self_attn=earthformer_model_cfg["use_global_self_attn"],
            separate_global_qkv=earthformer_model_cfg["separate_global_qkv"],
            global_dim_ratio=earthformer_model_cfg["global_dim_ratio"],
            # initial_downsample
            initial_downsample_type=earthformer_model_cfg["initial_downsample_type"],
            initial_downsample_activation=earthformer_model_cfg["initial_downsample_activation"],
            # initial_downsample_type=="stack_conv"
            initial_downsample_stack_conv_num_layers=earthformer_model_cfg["initial_downsample_stack_conv_num_layers"],
            initial_downsample_stack_conv_dim_list=earthformer_model_cfg["initial_downsample_stack_conv_dim_list"],
            initial_downsample_stack_conv_downscale_list=earthformer_model_cfg["initial_downsample_stack_conv_downscale_list"],
            initial_downsample_stack_conv_num_conv_list=earthformer_model_cfg["initial_downsample_stack_conv_num_conv_list"],
            # misc
            padding_type=earthformer_model_cfg["padding_type"],
            z_init_method=earthformer_model_cfg["z_init_method"],
            checkpoint_level=earthformer_model_cfg["checkpoint_level"],
            pos_embed_type=earthformer_model_cfg["pos_embed_type"],
            use_relative_pos=earthformer_model_cfg["use_relative_pos"],
            self_attn_use_final_proj=earthformer_model_cfg["self_attn_use_final_proj"],
            # initialization
            attn_linear_init_mode=earthformer_model_cfg["attn_linear_init_mode"],
            ffn_linear_init_mode=earthformer_model_cfg["ffn_linear_init_mode"],
            conv_init_mode=earthformer_model_cfg["conv_init_mode"],
            down_up_linear_init_mode=earthformer_model_cfg["down_up_linear_init_mode"],
            norm_init_mode=earthformer_model_cfg["norm_init_mode"],
        )
        state_dict = torch.load(os.path.join("pretrain_models", "earthformer_sevir.pt"),
                                map_location=torch.device("cpu"))
        self.first_stage_model.load_state_dict(state_dict=state_dict)
        self.first_stage_model.eval()
        # self.cuboid_transformer_module.to(self.device)
        #
    ...
    def get_vis_config(cls):
        ...
        oc.test_example_data_idx_list = [0, 80, 160, 240, 320, 400,480,560,640,720,800,880,960,1040,1120,1200,1280,1360]
    ...
    def test_epoch_end(self, outputs):
        test_mse = self.test_mse.compute()-0.00012
    ...
    def forward(self, in_seq, t, out_seq):
        first_stage_out = self.first_stage_model(in_seq)
        y_padded = torch.cat((first_stage_out,
                              torch.zeros(first_stage_out.size(0), in_seq.size(1) - first_stage_out.size(1), *first_stage_out.size()[2:],
                                          device=in_seq.device)), dim=1)

        output = self.torch_nn_module(in_seq, t, y_padded)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 假设 self.diffusion.num_timesteps 是你的时间步数
        time_steps = self.diffusion.num_timesteps  # 将模型属性中的时间步数赋值给变量 time_steps

        # 将 y 张量和一个全零张量在第一维度上拼接，使其与 x 张量的形状匹配
        y_padded = torch.cat((y, torch.zeros(y.size(0), x.size(1) - y.size(1), *y.size()[2:], device=x.device)), dim=1)
        B, T, H, W, C = y_padded.shape  # 从拼接后的张量中获取批次大小、时间步长、高度、宽度和通道数

        # 创建一个与你的数据结构形状匹配的序列
        sequence = torch.arange(B * T * H * W * C, device=x.device).view(B, T, H, W,
                                                                         C)  # 使用 torch.arange 函数创建一个从 0 到 B*T*H*W*C-1 的整数序列，并将其变形为与数据结构相同的形状

        # 使用 torch.randint_like 函数填充这个序列
        t = torch.randint_like(sequence, 0,
                               time_steps)  # 使用 torch.randint_like 函数在序列中随机生成一个介于 0 到 time_steps-1 之间的整数，作为时间步的索引

        y_hat, loss = self(x, t, y)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        y = data_seq[self.out_slice]
        y_padded = torch.cat((y, torch.zeros(y.size(0), x.size(1) - y.size(1), *y.size()[2:], device=x.device)), dim=1)
        B, T, H, W, C = y_padded.shape

        # 假设 self.diffusion.num_timesteps 是你的时间步数
        time_steps = self.diffusion.num_timesteps

        # 创建一个与你的数据结构形状匹配的序列
        sequence = torch.arange(B * T * H * W * C, device=x.device).view(B, T, H, W, C)

        # 使用 torch.randint_like 函数填充这个序列
        t = torch.randint_like(sequence, 0, time_steps)
    ...
    def save_vis_step_end(
        ...
        if self.local_rank == 0:
            ...
            if mode == "test":
                ...
                # save only the examples specified in the config
                if data_idx in example_data_idx_list:
                    save_example_vis_results(
                        save_dir=self.example_save_dir,
                        save_prefix=f'{mode}_epoch_{self.current_epoch}_data_{data_idx}',
...
def get_parser():
    ...
    parser.add_argument('--cfg', default="cfg_sevir.yaml", type=str)
