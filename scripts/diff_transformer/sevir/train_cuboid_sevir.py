...
class CuboidSEVIRPLModule(pl.LightningModule):
    ...
    def forward(self, in_seq, t, out_seq):
        y_padded = torch.cat((out_seq, torch.zeros(out_seq.size(0), in_seq.size(1) - out_seq.size(1), *out_seq.size()[2:], device=in_seq.device)), dim=1)
...
def get_parser():
    ...
    parser.add_argument('--ckpt_name', default="last.ckpt", type=str,
