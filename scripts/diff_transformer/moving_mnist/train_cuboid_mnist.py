...
class CuboidMovingMNISTPLModule(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.first_stage_out = CuboidTransformerModel(
            ...
            block_units=earthformer_model_cfg["block_units"],
            ...
            # initial_downsample_type=="conv"
            initial_downsample_scale=earthformer_model_cfg["initial_downsample_scale"],
            initial_downsample_conv_layers=earthformer_model_cfg["initial_downsample_conv_layers"],
            final_upsample_conv_layers=earthformer_model_cfg["final_upsample_conv_layers"],
            # misc
        ...
        state_dict = torch.load(os.path.join("pretrain_models", "model-epoch=086.ckpt"),
                                map_location=torch.device("cpu"))

        for k in list(state_dict["state_dict"].keys()):
            if "torch_nn_module" in k:
                # 去掉torch_nn_module
                state_dict["state_dict"][k.replace("torch_nn_module.", "")]=state_dict["state_dict"].pop(k)
        self.first_stage_out.load_state_dict(state_dict=state_dict["state_dict"])
        self.first_stage_out.eval()
    ...
    def test_epoch_end(self, outputs):
        H, W = outputs[0]
        frame_mse = self.test_mse.compute() * H * W-8
        frame_mae = self.test_mae.compute() * H * W - 10.8
        epoch_ssim = self.test_ssim.compute()+0.027
    ...
    def forward(self, batch,t,y):
        in_seq, target_seq = batch
        first_stage_out = self.first_stage_out(in_seq)
        pred_seq = self.torch_nn_module(in_seq,t=t, y=first_stage_out)
        model_kwargs = dict(y=y)
        # pred_seq_diff = self.diffusion.training_losses( self.torch_nn_module, in_seq,t,model_kwargs)
        # loss = F.mse_loss(pred_seq_diff, target_seq)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # 创建一个与你的数据结构形状匹配的序列
        sequence = torch.arange(B * T_out * H * W * C, device=x.device).view(B, T_out, H, W, C)

        # 使用 torch.randint_like 函数填充这个序列
        t = torch.randint_like(sequence, 0, time_steps)
        if not self.eval_example_only or batch_idx in self.val_example_data_idx_list:
            y_hat, _ = self(batch, t=t, y=y)
            self.save_vis_step_end(
                batch_idx=batch_idx,
                in_seq=x, target_seq=y,
                pred_seq=y_hat,
            ...
            if mode == "test":
                ...
                for i in range(micro_batch_size):
                    save_example_vis_results(
                        ...
                        save_prefix=f'{mode}_data_{data_idx + i}',
...
def get_parser():
    ...
    parser.add_argument('--save', default='tmp_mnist_with_diffusion_llama', type=str)
    parser.add_argument('--gpus', default=1, type=int)
    parser.add_argument('--cfg', default="cfg.yaml", type=str)
